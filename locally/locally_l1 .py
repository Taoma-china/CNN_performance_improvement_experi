...
def locally_model(initializers,def_lr,def_bcs,def_momen,drop,a,ak):
    
    model = Sequential()

    model.add(LocallyConnected2D(64,(3,3), input_shape=(16,16,1),init=initializers,kernel_regularizer=k,activity_regularizer=ak))
    model.add(Activation('relu'))
    model.add(Dropout(drop))

    model.add(LocallyConnected2D(32,(3,3),init=initializers,kernel_regularizer=k,activity_regularizer=ak))
    model.add(Dropout(drop))
    model.add(Flatten())
    model.add(Dense(10))
...
for j in range(9):# test l1
    ...
    b='s'#when test lr
    history=locally_model(initializers,def_lr,def_bcs,def_momen,drop,a,ak)
